...
\begin{multicols}{2}
	...
	\textbf{Uniforme discreta} $(a,b,n)$ con $-\infty<a\leq b<\infty$ estremi della progressione di n elementi

	\underline{Densità} $\frac{1}{n}$

	\underline{Media} $\frac{a+b}{2}$

	\underline{Varianza} $\frac{(a-b)^2}{12}\frac{n+1}{n-1} = \frac{n^2-1}{12}$

	\underline{Funzione caratteristica} $\Phi_X(t)=\frac{1}{n}e^{iat}\frac{1-e^{i \frac{n}{n-1}(b-a)t}}{1-e^{i \frac{1}{1n-1}(b-a)t}}$
	\\

	\textbf{Beta} $(\alpha,\beta)$ 

	\underline{Media} $\frac{\alpha}{\alpha+\beta}$

	\underline{Varianza} $\frac{\alpha\beta}{(\alpha+\beta)^2(\alpha+\beta+1)}$
...
$\mathbb{P}(\max(X_1,\dots,X_n)\leq t)=\prod_{i=1}^nF_{X_i}(t) \quad \mathbb{P}(\min(X_1,\dots,X_n)\leq t)=1-\prod_{i=1}^n(1-F_{X_i}(t))$ \quad se $X_i$ indipendenti!
...
N.B. \quad $L^p \Rightarrow \mathbb{P} \Rightarrow L$ \qquad $q.o. \Rightarrow \mathbb{P} \Rightarrow L$
...
\textbf{Stimatori di massima verosimiglianza} $\hat\alpha =\overline y-\hat\beta\overline x \quad \hat\beta=\frac{SS_{xy}}{SS_x} = \frac{\sum_i (x_i-\overline{x})y_i}{\sum_i (x_i-\overline{x})^2}$
...
\quad $\boldsymbol{y}=\boldsymbol{X\beta}+\boldsymbol{\epsilon}$ \quad $\boldsymbol{y}\sim \mathcal{N}(\boldsymbol{X\beta},\sigma^2 \boldsymbol{I}_n)$

\quad $\hat {\boldsymbol{\beta}}=(\boldsymbol{X}'\boldsymbol{X})^{-1}\boldsymbol{X}'\boldsymbol{y}$ \quad $\hat{\boldsymbol{\beta}}\sim\mathcal{N}(\boldsymbol{\beta},\sigma^2(\boldsymbol{X}'\boldsymbol{X})^{-1})$

\quad $\hat\sigma^2=\frac{(\boldsymbol{y}-\boldsymbol{X}\hat {\boldsymbol{\beta}})'(\boldsymbol{y}-\boldsymbol{X}\hat {\boldsymbol{\beta}})}{n}=\frac{\sum_{i=1}^n(y_i-\boldsymbol{x}_i\hat {\boldsymbol{\beta}})^2}{n}$

\includepdf[pages=2-6]{./PS_files/statistiche.pdf}

\includepdf[pages=1]{./PS_files/integrali.pdf}

\includepdf[pages=1]{./PS_files/taylor.pdf}

\includepdf[pages=1]{./PS_files/limiti.pdf}

\includepdf[pages=-]{./PS_files/serie.pdf}
